...
data class PeerInfo(
    ...
    /** Emergency-response node role announced via IdentityAnnouncement TLV 0x06. */
...
class PeerManager {
    ...
    private fun updatePeerInfoInternal(
        ...
        val roleChanged = existingPeer != null && existingPeer.role != role
        ...
        val shouldNotify = when {
            ...
            nicknameChanged || noiseKeyChanged || signingKeyChanged || connectedChanged ||
                capabilitiesChanged || announcementStateChanged || roleChanged -> true
            else -> false
        }
