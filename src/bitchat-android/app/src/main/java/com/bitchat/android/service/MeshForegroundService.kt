...
import com.bitchat.android.vlm.VlmApiService
import com.bitchat.android.vlm.VlmSettingsManager
...
class MeshForegroundService : Service() {
    ...
    private var vlmApiService: VlmApiService? = null
    private var updateJob: Job? = null
    private var cmonJob: Job? = null
    ...
    override fun onCreate() {
        ...
        // worker no-ops when the user has not enabled sync.
        try {
            com.bitchat.android.net.ConnectivityMonitor.getInstance(applicationContext).start()
            cmonJob = scope.launch {
                com.bitchat.android.net.ConnectivityMonitor.getInstance(applicationContext)
                    .hasInternet
                    .collect { online ->
                        if (online) {
            ...
            Log.w("MeshForegroundService", "Connectivity monitor init failed: ${e.message}")
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ACTION_STOP -> {
                ...
                cmonJob?.cancel()
                cmonJob = null
                try { com.bitchat.android.net.ConnectivityMonitor.getInstance(applicationContext).stop() } catch (_: Exception) { }
                try { com.bitchat.android.services.IngestWebServer.stop() } catch (_: Exception) { }
    ...
    private fun refreshLocalIngest() {
        try {
            val prefs = com.bitchat.android.services.SyncPreferences.getInstance(applicationContext)
            if (prefs.localIngestEnabled) {
                com.bitchat.android.services.IngestWebServer.start(applicationContext, prefs.localIngestPort)
            } else {
                com.bitchat.android.services.IngestWebServer.stop()
            }
        } catch (e: Exception) {
            Log.w("MeshForegroundService", "Local ingest refresh failed: ${e.message}")
        }
    }
    ...
    private fun startVlmApiServiceIfNeeded() {
        try {
            if (VlmSettingsManager.isEnabled()) {
                val port = VlmSettingsManager.getHttpPort()
                vlmApiService = VlmApiService(applicationContext, port)
                vlmApiService?.start()
                Log.i("MeshForegroundService", "VLM API started on port $port")
            }
        } catch (e: Exception) {
            Log.e("MeshForegroundService", "Failed to start VLM API: ${e.message}")
        }
    }

    private fun stopVlmApiService() {
        try {
            vlmApiService?.stop()
            vlmApiService = null
            Log.i("MeshForegroundService", "VLM API stopped")
        } catch (e: Exception) {
            Log.e("MeshForegroundService", "Failed to stop VLM API: ${e.message}")
        }
    }
