...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.SHELTER -> handleShelter(routed)
            MessageType.ACK -> handleAck(routed)
            else -> {
    ...
    /**
     * Handle shelter-registry gossip packets (public, broadcast).
     * Signed by originator's Ed25519 key — signature verified in [validatePacketSecurity].
     */
    private suspend fun handleShelter(routed: RoutedPacket) {
        delegate?.handleShelter(routed)
    }

    /**
     * Handle source-routed delivery acknowledgment packets (public, broadcast).
     * Carries a single [com.bitchat.android.protocol.MessageType.ACK] payload that the
     * sender uses to advance the BitchatMessage DeliveryStatus.
     */
    private suspend fun handleAck(routed: RoutedPacket) {
        delegate?.handleAck(routed)
    }
...
interface PacketProcessorDelegate {
    ...
    fun handleShelter(routed: RoutedPacket)
    fun handleAck(routed: RoutedPacket)
