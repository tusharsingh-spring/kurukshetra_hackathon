...
enum class MessageType(val value: UByte) {
    ...
    ACK(0x23u),  // Delivery confirmation for broadcast messages
    SHELTER(0x24u); // Shelter-registry gossip entry
...
data class BitchatPacket(
    ...
    var ttl: UByte,
    var route: List<ByteArray>? = null, // Optional source route hops: ordered list of peerIDs (8 bytes each), not including sender and final recipient
    /**
     * Emergency-response category tag carried by broadcast MESSAGE packets.
     * [Role.UNSET] (default) keeps the wire form byte-for-byte backward compatible
     * with prior clients: no [Flags.HAS_CATEGORY] flag is set and no category byte
     * is prepended to the payload.
     */
    val category: Role = Role.UNSET
) : Parcelable {
...
object BinaryProtocol {
    ...
    object Flags {
        ...
        const val HAS_CATEGORY: UByte = 0x10u  // Broadcast MESSAGE payload carries a 1-byte Role prefix
    }
    ...
    fun encode(packet: BitchatPacket, padding: Boolean = true): ByteArray? {
        try {
            ...
            // Prepend the emergency-response category byte (gated by HAS_CATEGORY flag) so the
            // 1-byte Role tag travels over the wire before any compression. The flag is also
            // set when role == UNSET? No — UNSET keeps the wire format byte-for-byte backward
            // compatible with prior clients (no flag, no prefix).
            val hasCategory = packet.category != com.bitchat.android.model.Role.UNSET
            if (hasCategory) {
                payload = byteArrayOf(packet.category.value.toByte()) + payload
            }
            ...
            if (hasCategory) {
                flags = flags or Flags.HAS_CATEGORY
            }
    ...
    private fun decodeCore(
        ...
        try {
            ...
            val hasCategory = (flags and Flags.HAS_CATEGORY) != 0u.toUByte()
            ...
            // Payload
            val rawPayload = if (isCompressed) {
            ...
            // Strip the 1-byte emergency-response category prefix if the flag is set.
            // The flag indicates the wire payload starts with [Role.value] followed by the
            // original content bytes; we move it to the model field so callers see content only.
            val category: com.bitchat.android.model.Role
            val strippedPayload: ByteArray
            if (hasCategory && rawPayload.isNotEmpty()) {
                category = com.bitchat.android.model.Role.fromValue(rawPayload[0].toUByte())
                strippedPayload = ByteArray(rawPayload.size - 1)
                System.arraycopy(rawPayload, 1, strippedPayload, 0, strippedPayload.size)
            } else {
                category = com.bitchat.android.model.Role.UNSET
                strippedPayload = rawPayload
            }
