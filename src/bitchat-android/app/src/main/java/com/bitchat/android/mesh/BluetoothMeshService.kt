...
class BluetoothMeshService(private val context: Context) : TransportBridgeService.TransportLayer {
    ...
    /**
     * Supplies the local node's emergency-response [com.bitchat.android.model.Role]
     * for the IdentityAnnouncement TLV 0x06. Wired by the chat UI so peers learn this
     * node's role on every announce / re-announce.
     */
    var roleProvider: (() -> com.bitchat.android.model.Role)? = null

    /**
     * Supplies the local node's subscribed-role filter set. Used by the
     * [ShelterAckCoordinator] to decide whether this node is the "reached responder"
     * for a broadcast message. Wired by the chat UI so BLE mirrors Wi-Fi Aware behavior.
     */
    var subscribedRolesProvider: (() -> Set<com.bitchat.android.model.Role>)? = null

    private val shelterAck: ShelterAckCoordinator by lazy {
        ShelterAckCoordinator(
            context = context.applicationContext,
            myPeerID = myPeerID,
            maxTtl = MAX_TTL,
            signBeforeBroadcast = ::signPacketBeforeBroadcast,
            broadcastPacket = { pkt -> broadcastRoutedPacket(RoutedPacket(pkt)) },
            sendToPeer = connectionManager::sendPacketToPeer,
            transportIdForBridge = "BLE",
            bridgeSendToPeer = { tid, peerID, pkt -> TransportBridgeService.sendToPeer(tid, peerID, pkt) },
            subscribedRolesProvider = { subscribedRolesProvider?.invoke() ?: emptySet() },
            lookupPeerInfo = peerManager::getPeerInfo,
            onShelterReceived = { shelter, verified -> delegate?.didReceiveShelter(shelter, verified) },
            onBroadcastAck = { id, acker, hops, reached -> delegate?.didReceiveBroadcastAck(id, acker, hops, reached) }
        )
    }

    fun broadcastShelter(shelter: com.bitchat.android.model.Shelter) {
        shelterAck.broadcastShelter(shelter)
    }

    fun broadcastShelterRemoval(shelter: com.bitchat.android.model.Shelter) {
        shelterAck.broadcastShelterRemoval(shelter)
    }

    fun gossipSheltersToPeer(peerID: String) {
        shelterAck.gossipSheltersToPeer(peerID)
    }
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(peerID: String, nickname: String, noisePublicKey: ByteArray, signingPublicKey: ByteArray, isVerified: Boolean, capabilities: com.bitchat.android.model.PeerCapabilities?, role: com.bitchat.android.model.Role): Boolean {
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
                ...
                DirectLinkAnnouncementPolicy.observationFor(routed, MAX_TTL)?.let { observation ->
                    if (connectionManager.observePeerIfCurrent(
                        ...
                        if (result.isFirst) {
                            serviceScope.launch { shelterAck.gossipSheltersToPeer(observation.peerID) }
                        }
                    } else {
            ...
            override fun handleMessage(routed: RoutedPacket) {
                ...
                try {
                    ...
                    if (isBroadcast && pkt.type == MessageType.MESSAGE.value) {
                        ...
                        shelterAck.handleMessageIngress(routed)
                    }
                } catch (_: Exception) { }
            }
            ...
            override fun handleShelter(routed: RoutedPacket) {
                shelterAck.handleShelterPacket(routed)
            }

            override fun handleAck(routed: RoutedPacket) {
                shelterAck.handleAckPacket(routed)
            }
        }
    ...
    fun sendMessage(
        ...
        channel: String? = null,
        category: com.bitchat.android.model.Role = com.bitchat.android.model.Role.UNSET,
        ...
        serviceScope.launch {
            val packet = BitchatPacket(
                ...
                timestamp = timestamp ?: System.currentTimeMillis().toULong(),
    ...
    fun sendBroadcastAnnounce() {
        serviceScope.launch {
            ...
            val announcement = IdentityAnnouncement.forLocalPeer(
                ...
                staticKey,
                signingKey,
                roleProvider?.invoke() ?: com.bitchat.android.model.Role.UNSET
            )
