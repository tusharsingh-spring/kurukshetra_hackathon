...
class MeshCore(
    ...
    data class Hooks(
        ...
        /** Supplies the local node's emergency-response [Role] for announcement TLV 0x06. */
        val localRoleProvider: (() -> com.bitchat.android.model.Role)? = null,
        /** Supplies the local node's subscribed-role filter set (for broadcast-ack responder detection). */
        val subscribedRolesProvider: (() -> Set<com.bitchat.android.model.Role>)? = null
    )
            ...
            override suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
                ...
                if (result.isFirst) {
                    ...
                    if (peerID != null && peerID != myPeerID) {
                        scope.launch { gossipSheltersToPeer(peerID) }
                    }
                }
                return true
            }
    ...
    private val shelterRegistry by lazy {
        try {
            com.bitchat.android.services.ShelterRegistry.getInstance(context.applicationContext)
    ...
    private val shelterAck: ShelterAckCoordinator by lazy {
        ShelterAckCoordinator(
            ...
            broadcastPacket = { pkt -> dispatchGlobal(RoutedPacket(pkt)) },
            sendToPeer = { peerID, pkt -> transport.sendPacketToPeer(peerID, pkt) },
            transportIdForBridge = transport.id,
            bridgeSendToPeer = { tid, peerID, pkt ->
                com.bitchat.android.service.TransportBridgeService.sendToPeer(tid, peerID, pkt)
            },
            subscribedRolesProvider = { hooks.subscribedRolesProvider?.invoke() ?: emptySet() },
    ...
    fun sendBroadcastAnnounce() {
        scope.launch {
            ...
            val announcement = IdentityAnnouncement.forLocalPeer(
                ...
                staticKey,
                signingKey,
                hooks.localRoleProvider?.invoke() ?: com.bitchat.android.model.Role.UNSET
            )
