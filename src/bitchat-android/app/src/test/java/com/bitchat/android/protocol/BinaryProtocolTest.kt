...
class BinaryProtocolTest {
    ...
    @Test
    fun `category flag preserves role byte and strips it from decoded payload`() {
        val original = makePacket(
            payload = "Reporting fire near 4th and Main".toByteArray(),
            category = com.bitchat.android.model.Role.FIRE
        )

        val encoded = BinaryProtocol.encode(original, padding = false)
        assertNotNull("Encoding must not return null", encoded)
        // v1 header byte indices: 0=version,1=type,2=ttl,3..10=timestamp(8B),11=flags,12..13=length
        val flags = encoded!![11]
        assertTrue("HAS_CATEGORY flag must be set when role != UNSET", (flags.toInt() and 0x10) != 0)
        ...
        // The decoded payload must NOT contain the prepended role byte; callers see content only.
        assertEquals(
            "stripped payload equals the original UTF-8 text",
            "Reporting fire near 4th and Main",
            String(decoded.payload, Charsets.UTF_8)
        )
    }

    @Test
    fun `UNSET category keeps the wire packet byte-identical to legacy clients`() {
        val legacy = makePacket(payload = "legacy broadcast".toByteArray())
        val withUnset = makePacket(
            payload = "legacy broadcast".toByteArray(),
            category = com.bitchat.android.model.Role.UNSET
        )

        val legacyEncoded = BinaryProtocol.encode(legacy, padding = false)!!
        val unsetEncoded = BinaryProtocol.encode(withUnset, padding = false)!!

        assertArrayEquals(
            "UNSET category must not perturb legacy wire bytes",
            legacyEncoded,
            unsetEncoded
        )
        assertEquals(
            com.bitchat.android.model.Role.UNSET,
            BinaryProtocol.decode(unsetEncoded)!!.category
        )
    }
